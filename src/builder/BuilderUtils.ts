...
export function generateLinks(pluginShortName: string, groups: IEntityGroups) {
    groups.clazz.forEach(value => entityToLink.set(value, `/${linkBase}/${pluginShortName}/${value.toLowerCase()}`));
    groups.typedef.forEach(value => {
        entityToLink.set(value, `/${linkBase}/${pluginShortName}/${typeDefSlug}#${value}`);
    });
